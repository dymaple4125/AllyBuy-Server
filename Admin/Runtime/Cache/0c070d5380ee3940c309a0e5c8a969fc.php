            ...
            $(function(){
                ...
                var table = $('#datatable').DataTable({
                    ...
                    columns: [
                        ...
                        { "mRender": function(data, type, full) {
                                var img  =   full['img'];
                                return "<img height='200' src='__ROOT__/public/uploads/promotion/"+img+"'>";
                                //return "<a href='__ROOT__/public/uploads/promotion/"+img+"' target='_blank'>"+img+"</a>";
                            }
                        }
                    ],
                    columndefs:[
                ...
                $('#btn_edit').click(function(){
                   ...
                   if(table.row('.selected').length==0){
                    ...
                    location.href   =   "<?php echo U('/promotion/edit/promotion_id/"+id+"', '', '');?>";
                });
                ...
                $('#btn_delete').click( function () {
                    ...
                    $('#remove_modal .modal-body p').html("确定要删除这个图片");
                    $('#remove_modal button[name=remove_btn]').unbind("click");
                    $('#remove_modal button[name=remove_btn]').click(function(){
                        $('#remove_modal').modal('hide');
                        //post data to delete
                        $.post("<?php echo U('delete', '', '');?>", {promotion_id:table.cell('.selected', 0).data()},
                        function(data){
                            //delete fail
                            if(data['status']==0)
                            {
                                $('#alert_fail').html("删除不成功");
                                $('#alert_fail').attr('hidden', false);
                            }
                            else if(data['status']==1)
                            {
                                $('#alert_success').html("删除成功");
