    ...
    <head>
        ...
        <script>
            ...
            function edit(country_id){
                location.href   =   "<?php echo U('/country/edit/country_id/"+country_id+"', '', '');?>";
            }
            
            //function to view
            function view(country_id){
                location.href   =   "<?php echo U('/country/view/country_id/"+country_id+"', '', '');?>";
            }
            
            function nav(country_id){
                location.href   =   "<?php echo U('/district/viewall/country_id/"+country_id+"', '', '');?>";
            }
    
            function del(country_id, country_name){
                
                clearStatus();
                
                $('#remove_modal .modal-body p').html("确定要删除《"+country_name+"》吗?");
                $('#remove_modal button[name=remove_btn]').unbind("click");
                $('#remove_modal button[name=remove_btn]').click(function(){
                    $('#remove_modal').modal('hide');
                    //post data to delete
                    $.post("<?php echo U('delete', '', '');?>", {country_id:country_id},
                    function(data){
                        //delete fail
                        if(data['status']==0)
                        {
                            $('#alert_fail').html("删除《"+country_name+"》不成功");
                            $('#alert_fail').attr('hidden', false);
                        }
                        else if(data['status']==1)
                        {
                            $('#alert_success').html("删除《"+country_name+"》成功");
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header" style="background-color:  darkred;">
                    <h4 class="modal-title" style="color: white;" >删除country</h4>
                </div>
                ...
                <div class="col-sm-10 col-md-10 main">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-condensed" style="text-align: center;">
                            <?php if(is_array($country_list)): foreach($country_list as $key=>$country): ?><tr id="country<?php echo ($country['country_id']); ?>">
                                    <td style="color: lightgrey;">#<?php echo ($country['country_id']); ?></td>
                                    <td style="width: 80%;"><a href="javascript:nav(<?php echo ($country['country_id']); ?>);"><?php echo ($country['country_name']); ?></a></td>
                                    <td>
                                        <div class="btn-group btn-group-sm" role="group"  aria-label="todo">
                                            <button type="button" class="btn btn-default" onclick="javascript:view(<?php echo ($country['country_id']); ?>);">
                                                <span class="glyphicon glyphicon-file" aria-hidden="true"></span>
                                            </button>
                                            <button type="button" class="btn btn-default" onclick="javascript:edit(<?php echo ($country['country_id']); ?>);">
                                                <span class="glyphicon glyphicon-edit" aria-hidden="true"></span>
                                            </button>
                                            <button type="button" class="btn btn-danger" onclick="javascript:del(<?php echo ($country['country_id']); ?>,'<?php echo ($country['country_name']); ?>');">
